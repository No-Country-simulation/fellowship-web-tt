...
| Capturas de producto | `/public/product/*.webp` | Cómo funciona (`perfil-*.webp`). El hero usa fichas HTML, no captura. `quality={90}` (allowlist `images.qualities` en `next.config.ts`). Fotos en WebP; avatares y logos siguen en SVG |
