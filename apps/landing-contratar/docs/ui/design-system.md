...
| Capturas de producto | `/public/product/*` | Cómo funciona: `perfil-*.webp` (`quality={90}`, allowlist en `next.config.ts`). Hero: fichas HTML + `avatar-*.webp` (hexágono `.hero-profile-avatar`, `unoptimized`). Quotes: avatares SVG. Logos de aliados en SVG. Wordmark No Country: PNG (`/brand/logo-no-country.png`) |
