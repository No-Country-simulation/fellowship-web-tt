...
| `GEMINI_MODEL` | Next.js `lib/agent` | Optional primary model (default `gemini-3.1-flash-lite`) |
| `GEMINI_FALLBACK_MODEL` | Next.js `lib/agent` | Optional fallback (default `gemini-2.5-flash-lite`) |
| `GEMINI_FIDELITY_LLM` | Next.js `lib/agent` | Optional `true` to run a second 2.5 fidelity check |
...
Text-only drafts in **first-person testimonio voice**; FFmpeg owns media binaries.
Primary model falls back to 2.5 Flash-Lite on 503/429/empty. Optional `GEMINI_FIDELITY_LLM=true` adds a second fidelity pass.
