...
- Acciones destructivas (eliminar, cancelar, rechazar)
...
| `gradient` | CTA destacado (Brand Gradient Primary) |
...
- `variant="gradient"` usa `text-white`. En una banda `surface="light"` el `color` heredado pisa esa utilidad: el `globals.css` de la landing fuerza blanco otra vez en `.bg-brand-gradient`.
...
- Para íconos, Lucide ya está en las apps (`lucide-react`).
